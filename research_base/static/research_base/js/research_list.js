...
area_name_input = document.getElementById("area_name_input")
...
for (var i = 0; i < edit_links.length; i++) {
    edit_links[i].onclick = function () {
        id = this.value
        res_name = document.getElementById("td_name_" + id).textContent.trim();
        area_name = document.getElementById("td_area_name_" + id).textContent.trim();
        main_form.action = 'http://localhost:8000/research_edit/' + id
        main_form.style.display = 'block'
        table_container.style.display = 'none'
        add_btn.textContent = 'Изменить'

        name_input.value = res_name
        area_name_input.value = area_name
    }
}


document.getElementById("create").onclick = function () {
    main_form.action = 'http://localhost:8000/research_create/'
    main_form.style.display = 'block'
