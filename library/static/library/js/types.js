...
edit_type_forms = document.getElementsByClassName("edit_type_form")
...
create_form = document.getElementById("create_type_form")
...
for (var i = 0; i < edit_links.length; i++) {
    edit_links[i].onclick = function () {
        ...
        id = this.value
        edit_form = document.getElementById("edit_type_form_" + id)
...
for (var i = 0; i < delete_links.length; i++) {
    delete_links[i].onclick = function () {
        ...
        delete_button.href = 'http://localhost:8000/library/delete_type/' + id
    }
